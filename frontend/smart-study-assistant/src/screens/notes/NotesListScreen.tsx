...
const NotesListScreen = ({ navigation }: any) => {
  ...
  // CRUD
  const handleAdd = () => {
    const validTag = tags.includes(temp.tag) && temp.tag !== 'Tất cả' ? temp.tag : tags[1];
    setNotes([{ title: temp.title, tag: validTag, date: new Date().toISOString().slice(0, 10) }, ...notes]);
    closeModal();
  };
  const handleEdit = () => {
    if (modal.index !== undefined) {
      ...
      newNotes[modal.index] = { ...newNotes[modal.index], title: temp.title, tag: validTag };
      setNotes(newNotes);
    }
    closeModal();
  };
  ...
  // Custom Tag Picker for Modal
  const TagPicker = ({ value, onChange }: { value: string, onChange: (v: string) => void }) => (
      ...
      <ModalCard
        ...
        fields={[
          { label: 'Tiêu đề', value: temp.title, onChange: v => setTemp(t => ({ ...t, title: v })) },
          {
            label: 'Tag',
            value: temp.tag,
            render: () => <TagPicker value={temp.tag} onChange={v => setTemp(t => ({ ...t, tag: v }))} />
          },
        ]}
        onSubmit={handleAdd}
        onCancel={closeModal}
