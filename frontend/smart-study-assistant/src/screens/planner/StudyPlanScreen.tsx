...
const StudyPlanScreen = ({ navigation }: any) => {
  ...
  const fetchStudyPlans = async () => {
    ...
    try {
      ...
      Alert.alert(
        t("study_plan.error"),
        error.message || t("study_plan.loadError")
      );
    } finally {
      setLoading(false);
    }
  ...
  const openAdd = () => {
    ...
    const currentTime = `${currentHour}:${currentMinute
      .toString()
      .padStart(2, "0")}`;
    const nextTime = `${currentHour}:${(currentMinute + 30)
      .toString()
      .padStart(2, "0")}`;
  ...
  const handleAdd = async () => {
    ...
    try {
      ...
      Alert.alert(
        t("study_plan.error"),
        error.message || t("study_plan.addError")
      );
    } finally {
  ...
  const handleEdit = async () => {
    ...
    if (modal.index !== undefined && sessions[modal.index]) {
      ...
      try {
        ...
        Alert.alert(
          t("study_plan.error"),
          error.message || t("study_plan.updateError")
        );
      } finally {
        setOperationLoading(false);
      }
  ...
  const handleDelete = async () => {
    if (modal.index !== undefined && sessions[modal.index]) {
      ...
      try {
        ...
        Alert.alert(
          t("study_plan.error"),
          error.message || t("study_plan.deleteError")
        );
      } finally {
        setOperationLoading(false);
      }
  ...
  const handleToggleCompletion = async (session: StudyPlan) => {
    try {
      ...
      Alert.alert(
        t("study_plan.error"),
        error.message || t("study_plan.toggleError")
      );
    }
  };
  ...
  const renderSession = ({
    ...
    <View
      style={[styles.sessionCard, item.completed && styles.completedSession]}
    >
      <TouchableOpacity
      ...
      <View style={styles.sessionContent}>
        <Text
          style={[styles.sessionTitle, item.completed && styles.completedText]}
        >
          {item.title}
        </Text>
        <Text
          style={[styles.sessionDesc, item.completed && styles.completedText]}
        >
        ...
        {item.note && (
          <Text
            style={[styles.sessionNote, item.completed && styles.completedText]}
          >
            {item.note}
          </Text>
        )}
    ...
    <View style={styles.container}>
      ...
      {sessions.length === 0 ? (
        ...
        >
          ...
          <TouchableOpacity style={styles.createFirstBtn} onPress={openAdd}>
            <Text style={styles.createFirstText}>
              {t("study_plan.add_first_study_plan")}
            </Text>
          </TouchableOpacity>
        </View>
      ) : (
        <FlatList
          ...
          ListEmptyComponent={
            <Text style={styles.empty}>{t("study_plan.no_sessions")}</Text>
          }
      ...
      <ModalCard
        ...
        fields={[
          {
            label: t("study_plan.planTitle"),
            value: temp.title,
            onChange: (v) => setTemp((t) => ({ ...t, title: v })),
          },
          {
            label: t("study_plan.note"),
            value: temp.note,
            onChange: (v) => setTemp((t) => ({ ...t, note: v })),
            multiline: true,
          },
        ]}
        extraContent={
      ...
      {operationLoading && (
        <View style={styles.loadingOverlay}>
          <ActivityIndicator size="large" color={currentTheme.colors.primary} />
          <Text
            style={{ marginTop: 8, color: currentTheme.colors.textSecondary }}
          >
            {t("study_plan.processing")}
          </Text>
        </View>
      )}
    </View>
