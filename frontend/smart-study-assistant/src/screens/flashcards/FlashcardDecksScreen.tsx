...
const FlashcardDecksScreen = ({ navigation }: any) => {
  ...
  const [temp, setTemp] = useState<Partial<Deck>>({
    title: "",
    description: "",
    tags: [],
    isPublic: false,
  ...
  const fetchDecks = async () => {
    ...
    try {
      ...
      Alert.alert(
        t("flashcards.error"),
        error.message || t("flashcards.loading")
      );
    } finally {
      setLoading(false);
    }
  ...
  const handleAdd = async () => {
    if (!temp.title?.trim()) {
      Alert.alert(t("flashcards.error"), t("flashcards.deckName"));
      return;
    }
  ...
  const handleEdit = async () => {
    ...
    if (modal.index !== undefined && decks[modal.index]) {
      ...
      try {
        ...
        setDecks(newDecks);
        closeModal();
        Alert.alert(t("flashcards.success"), t("flashcards.deckUpdated"));
      } catch (error: any) {
        Alert.alert(
          t("flashcards.error"),
          error.message || t("flashcards.error")
        );
      } finally {
        setOperationLoading(false);
      }
  ...
  const handleStudyPress = (deck: Deck) => {
    console.log(
      "Navigating to FlashcardPractice with deck:",
      deck.id,
      deck.title
    );
    ...
    <View style={styles.container}>
      ...
      {decks.length === 0 ? (
        ...
        >
          ...
          >
            {tab === "my" ? t("flashcards.noDecks") : t("flashcards.noDecks")}
          </Text>
          {tab === "my" && (
      ...
      <ModalCard
        ...
        fields={[
          ...
          {
            label: t("flashcards.description"),
            value: temp.description || "",
            onChange: (v) => setTemp((t) => ({ ...t, description: v })),
          },
        ]}
