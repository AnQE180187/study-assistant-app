          ...
          <View style={styles.card}>
            ...
            >
                    ...
                    <View style={styles.categoryContainer}>
                      <ScrollView horizontal showsHorizontalScrollIndicator={false}>
                        {f.categories.map((cat) => (
                          <TouchableOpacity
                            key={cat}
                            style={[styles.categoryBtn, f.value === cat && styles.categoryBtnActive]}
                            onPress={() => f.onChange(cat)}
                          >
                            <Text style={[styles.categoryText, f.value === cat && styles.categoryTextActive]}>
                              {cat}
                            </Text>
                          </TouchableOpacity>
                        ))}
                      ...
                      {f.onAddCategory && (
                        <TouchableOpacity onPress={f.onAddCategory} style={styles.addCategoryBtn}>
              ...
              {type !== 'delete' && typeof isPublic === 'boolean' && onTogglePublic && (
                <TouchableOpacity style={styles.toggleRow} onPress={onTogglePublic}>
                  <Ionicons name={isPublic ? 'earth' : 'lock-closed'} size={20} color={COLORS.primary} style={{ marginRight: 6 }} />
                  <Text style={styles.toggleText}>{isPublic ? 'Công khai' : 'Riêng tư'}</Text>
                </TouchableOpacity>
              )}
              {/* Toggle Pinned */}
              {type !== 'delete' && typeof isPinned === 'boolean' && onTogglePinned && (
                <TouchableOpacity style={styles.toggleRow} onPress={onTogglePinned}>
                  <Ionicons name={isPinned ? 'pin' : 'pin-outline'} size={20} color={COLORS.primary} style={{ marginRight: 6 }} />
                  <Text style={styles.toggleText}>{isPinned ? 'Đã ghim' : 'Ghim'}</Text>
                </TouchableOpacity>
              )}
              {type === 'delete' && (
